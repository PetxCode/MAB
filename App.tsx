...
const App: React.FC = () => {
  ...
  }, [appState, selectedCourse, questions, userAnswers, currentSession, currentModule, quizExpiryTime, isHydrated]);
  ...
  const startQuiz = async (moduleName: string, sessionName: string) => {
    ...
    try {
      ...
      const selected = shuffled.slice(0, Math.min(pool.length, QUESTIONS_PER_QUIZ));
      ...
      console.error(err);
      setErrorMsg("Failed to generate questions. Please ensure the API Key is valid and try again.");
      setAppState(AppState.SETUP);
    }
  };
  ...
  const handleResumeSession = () => {
    ...
    if (questions.length > 0) {
       ...
       if (appState === AppState.REVIEW || (!quizExpiryTime && Object.keys(userAnswers).length > 0)) {
         setAppState(AppState.REVIEW);
       } else {
         setAppState(AppState.ACTIVE);
       }
  ...
  const activeSessionLabel = selectedCourse ? `${selectedCourse.code} - ${currentSession}` : 'Session';
    ...
    <div className="h-screen w-full bg-slate-50 flex flex-col font-sans">
      ...
      <div className="bg-slate-900 text-white px-6 py-4 shadow-lg flex justify-between items-center flex-shrink-0 z-30 border-b border-slate-800">
        ...
        >
          ...
          <span className="font-semibold tracking-tight">BUAD E-Library CBT</span>
        </div>
        ...
        {errorMsg && (
          <div className="absolute top-6 left-0 right-0 z-50 px-6 flex justify-center">
             <div className="max-w-xl w-full bg-white border-l-4 border-red-500 text-slate-700 px-6 py-4 rounded-r-lg shadow-xl flex items-start justify-between">
                <div>
                  <h4 className="font-bold text-red-600 mb-1">Error Encountered</h4>
        ...
        <div className="h-full overflow-auto scroll-smooth">
          ...
          {appState === AppState.SETUP && selectedCourse && (
            <SetupScreen 
              ...
              activeSessionLabel={hasActiveSession && selectedCourse.id === selectedCourse.id ? activeSessionLabel : null}
              onResume={handleResumeSession}
            />
          )}
